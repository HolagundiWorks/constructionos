...
## ACO project notes (how this repo applies the above)
...
- **Navigation shell:** the "use `NavigationView` for primary navigation" rule is
  ...
  (section tab strip + an icon command band, no dropdowns), and `NavigationView`
  Top mode and `SelectorBar` both **native-crash** on the pinned Windows App SDK
  (WINUI3-MIGRATION §9 fix 7). The ribbon is **composed from stock controls**
  (`ToggleButton`, `AppBarButton`, `FontIcon`) — no custom template, no
  owner-draw — so it still honours "stock controls only".
- **TFM / SDK:** the project targets `net8.0-windows10.0.19041.0` on
  `Microsoft.WindowsAppSDK 1.5.240627000` (not net10 / latest). Both are pinned
  for stability (LiveCharts `rc3.3` keeps SkiaSharp 2.88 + WindowsAppSDK 1.5);
