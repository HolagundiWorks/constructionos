...
| **U4** | Dashboard (KPI cards, `InfoBar` advisories) + **charts** (EVM/cash/ageing) | **🚧 Partial** — **Home dashboard: stock KPI stat cards** (money snapshot from `/api/kpi`) + **advisories as `InfoBar`s** (`/api/dashboard`); **Charts page plots a real LiveCharts column series** (SkiaSharp native verified); EVM/ageing chart series pending |
