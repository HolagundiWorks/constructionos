...
| **U4** | Dashboard (KPI cards, `InfoBar` advisories) + **charts** (EVM/cash/ageing) | **✅ built** — Home **KPI stat cards** + **`InfoBar` advisories**; Charts page: money-KPI column series **+ cash-flow forecast** (in/out columns + running-balance line, `/api/cashflow` labels/values/series — cloud CT-4); EVM per-project CPI/SPI chart (`/api/evm`). Ageing chart is the one remaining series |
