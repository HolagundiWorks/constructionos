...
| **U3** | Money/Billing/Purchases pages; forms with stock inputs; `CommandBar` | **🚧 Partial** — **Payments: metadata-driven "New payment" form** (`CommandBar` + `ContentDialog`, fields from `GET /api/payments`, `POST` → 201, posts to the ledger — verified end-to-end); Capture + Import pages built; the same form pattern extends to invoices/bills next |
| **U4** | Dashboard (KPI cards, `InfoBar` advisories) + **charts** (EVM/cash/ageing) | **✅ mostly built** — **Home dashboard: stock KPI stat cards** (`/api/kpi`) + **advisories as `InfoBar`s** (`/api/dashboard`); **Charts page** money-KPI LiveCharts column series (SkiaSharp native verified); **EVM page**: per-project CPI/SPI grouped column chart + portfolio summary (`/api/evm`). Cash-flow / ageing chart series still pending (needs cloud CT-4 chart shapes) |
