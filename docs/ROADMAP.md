...
| **U5** | Controls (Risk/Opportunity/Lessons/Submittals), Process, search; **nav-complete** — every catalog tab opens an API-backed page (masters / money / registers / reports); **GST & TDS**, **Weekly Review**, **Accounting** (P&L / Balance Sheet / Journal) + **Look-ahead** (PPC). *Not yet workflow-parity with tkinter for MB/RA/muster/GRN/Assistant* — see [`COMPLETENESS-AUDIT.md`](COMPLETENESS-AUDIT.md) | ✅ built + runs (nav-complete) |
...
| **U7** | Persona menus ✅, accessibility (`AutomationProperties`) ✅ mechanical, **nav-complete ✅** (every catalog tab is a live page — Tools, Controls, EVM/Portfolio/Productivity real); **workflow parity** still open (miswires + shallow DataTable tabs — audit §4); remaining: interactive a11y walkthrough, retire tkinter on Windows | 🚧 in progress |
...
- ✅ **Nav-complete pass** — every `SECTIONS_CATALOG` tab now resolves to a real page (master / money doc / register / dedicated report). The last always-on gap, **Tools**, is wired: a `ToolsPage` over new `/api/firm` + `/api/modules` (firm letterhead identity + module on/off toggles, CSRF-gated). Still desktop/browser-only (not in the JSON API): backup/restore, invoice-number series, reference-data load, and language — a future Tools follow-up. **Workflow parity** (purpose-built MB/RA, muster, GRN match, Assistant, cash-flow forecast, subcontractor WO) is tracked in [`COMPLETENESS-AUDIT.md`](COMPLETENESS-AUDIT.md) + [`RESEARCH-OPTIMISATION-AND-PRECISION.md`](RESEARCH-OPTIMISATION-AND-PRECISION.md) — do not equate “live page” with desktop-depth.
