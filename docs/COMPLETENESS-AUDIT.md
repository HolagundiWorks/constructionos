...
> **Fixes applied since this audit (local + cloud, API now u0.12):**
> - **Assistant** no longer lands on the sidecar page — a real **AssistantPage**
>   over new `GET /api/assistant/quick` (exact no-model figures) +
>   `POST /api/assistant` (text-to-SQL turn; honest "turn on the AI engine" when
>   the model is off, never a fake answer).
> - **Cash Flow** no longer lands on the payments list — a dedicated
>   **CashFlowPage** over `/api/cashflow`: in/out/balance chart, opening/closing
>   cards, and the flagged first week the balance goes negative.
> - The crude **text-dump** pages (Risk/Opportunity/Lessons/Submittals registers,
>   the EVM per-project detail, Portfolio, Productivity) were replaced with real
>   tables + headline cards (were `Ui.Lines` "key: value" strings).
> - **Tools** firm-details + module toggles shipped (`/api/firm`, `/api/modules`).
...
|---|---|---|---|
| **Assistant** | ~~`CapturePage`~~ → **`AssistantPage`** ✅ FIXED | RAG text-to-SQL assistant (`tab_assistant.py`) | ~~High~~ done |
| **AI Engine** | `CapturePage` | Foundry start/stop (`tab_aiengine.py`) | **High** (not in current menu) |
