...
def outward(conn, month):
    """Sales / output GST from tax invoices -> (rows, totals)."""
    where, params = _month_clause('ti.invoice_date', month)
    rows, tot = [], {'taxable': 0.0, 'cgst': 0.0, 'sgst': 0.0, 'igst': 0.0, 'total': 0.0}
    sql = ("SELECT ti.invoice_no, ti.invoice_date, c.name AS party, "
           "ti.subtotal, ti.gst_pct, ti.interstate, ti.total_amount "
           "FROM tax_invoices ti LEFT JOIN clients c ON c.id = ti.client_id"
           + where + " ORDER BY ti.invoice_date, ti.id")
...
def build_gst_tab(parent, db_getter):
    ...
    inv_headers = ('Invoice No', 'Date', 'Party', 'Taxable', 'CGST', 'SGST',
                   'IGST', 'Total')
    nb.add(ReportView(nb, db_getter, 'Output GST (Sales)', inv_headers,
